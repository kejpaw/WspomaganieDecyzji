...
export class MainComponent implements OnInit {
    ...
    submitMarkers() {
        ...
        if (this.map.startMarker && this.map.endMarker)
        {
            ...
            var order = {
                ...
                Request: parseInt(e.options[e.selectedIndex].value)              
            };
    ...
    count() {
         this._httpService.get('/api/values/GetAstar').subscribe(values => {
           // var object = JSON.parse(values.text());
            /*
            object.forEach(function (item) {
                me.map.addCustomMarker(item.StartLat as number, item.StartLng as number, RideStatus.manStart);
    ...
    deleteAll() {
        this._httpService.get('/api/values/ClearAll').subscribe(values => {
            alert('Usunięto historię');
           // var object = JSON.parse(values.text());
            /*
            object.forEach(function (item) {
                me.map.addCustomMarker(item.StartLat as number, item.StartLng as number, RideStatus.manStart);
